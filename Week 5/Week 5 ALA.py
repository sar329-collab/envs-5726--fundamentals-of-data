...
with open(csv_path, 'r', encoding='cp1252') as csv_file:
    ...
    for row in reader:
        ...
        if country_of_destination not in Countries_by_deforestation_dict:
            Countries_by_deforestation_dict[country_of_destination] = [cocoa_deforestation_15_years_total_exposure]
        else:
            Countries_by_deforestation_dict[country_of_destination].append(cocoa_deforestation_15_years_total_exposure)
...
with open(csv_path, 'r', encoding='cp1252') as csv_file:
    ...
    for row in reader:
        ...
        if country_of_destination not in Countries_by_net_emissions_dict:
            Countries_by_net_emissions_dict[country_of_destination] = [cocoa_net_emissions_15_years_total]
        else:
            Countries_by_net_emissions_dict[country_of_destination].append(cocoa_net_emissions_15_years_total)
...
def get_summary_data(summary_dict,):
    ...
    for country_of_destination in summary_dict:
        ...
        total = sum(float(values))
        summary_table.append([country_of_destination, total])
        total.append(country_of_destination)
...
with open(export_csv_path, 'w', newline='', encoding='cp1252') as file:
    writer = csv.writer(file)
    writer.writerows([data_headers]+summary_deforestation)

export_csv_path = (Path(r'/Users/shaunroberts/Desktop/ENVS 5726/week 5/Summary_Net_Emissions.csv'))
with open(export_csv_path, 'w', newline='', encoding='cp1252') as file:
    writer = csv.writer(file)
    writer.writerows([data_headers]+summary_emissions)
